...
describe Nanoc::Core::OutdatednessChecker do
  ...
  let(:items_before_array) do
    [item_home_before, item_articles_before, item_article_a_before, item_article_b_before,
    ...
    context 'when articles item depends on compiled content of articles' do
      let(:dependency_store) do
        Nanoc::Core::DependencyStore.new(
          ...
          store.record_dependency(item_articles_before, item_article_a_before,
                                  compiled_content: true)
          store.record_dependency(item_articles_before, item_article_b_before,
                                  compiled_content: true)
          store.record_dependency(item_articles_before, item_article_c_before,
                                  compiled_content: true)
        end
      end
  ...
  context 'when an item is added to the site' do
    ...
    context 'when articles item has a specific attribute dependency on all items, and attribute matches' do
      before do
        ...
        old_dependency_store.record_dependency(item_articles_before, items_before_coll,
  ...
  context 'when a layout is added to the site' do
    ...
    context 'when articles item has a specific attribute dependency on all layouts, and attribute matches' do
      before do
        ...
        old_dependency_store.record_dependency(item_articles_before, layouts_before_coll,
                                               attributes: { kind: 'article' })
        old_dependency_store.store
