...
class Nanoc::Helpers::XMLSitemapTest < Nanoc::TestCase
  ...
  def test_xml_sitemap
    if_have 'nokogiri' do
      ...
      # Create item 1
      item = Nanoc::Core::CompilationItemView.new(Nanoc::Core::Item.new('some content 1', {}, '/item-one'),
                                                  @view_context)
      items << item
      ...
      item = Nanoc::Core::CompilationItemView.new(
        Nanoc::Core::Item.new('some content 2', { is_hidden: true }, '/item-two'), @view_context
      )
      items << item

      # Create item 3
      attrs = { mtime: Time.parse('2004-07-12 00:00:00 +02:00'), changefreq: 'daily', priority: 0.5 }
      item = Nanoc::Core::CompilationItemView.new(Nanoc::Core::Item.new('some content 3', attrs, '/item-three'),
                                                  @view_context)
      items << item
      ...
      # Create item 4
      item = Nanoc::Core::CompilationItemView.new(Nanoc::Core::Item.new('some content 4', {}, '/item-four'),
                                                  @view_context)
      items << item
      ...
      # Create sitemap item
      @item = Nanoc::Core::CompilationItemView.new(Nanoc::Core::Item.new('sitemap content', {}, '/sitemap'),
                                                   @view_context)
  ...
  def test_url_escape
    if_have 'nokogiri' do
      ...
      assert_equal 1, urls.size
      assert_equal 'http://example.com/cool%20projects/%D0%BF%D1%80%D0%BE%D0%B2%D0%B5%D1%80%D0%BA%D0%B0',
                   urls[0].css('> loc').inner_text
    end
  end
